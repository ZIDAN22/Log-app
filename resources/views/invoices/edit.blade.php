    ...
    <div class="max-w-7xl mx-auto">
        <div class="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            ...
            <a href="{{ route('invoices.index') }}"
                ...
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="h-4.5 w-4.5 text-slate-700"
        ...
        <div class="grid gap-6 xl:grid-cols-[1.8fr_1fr]">
                    ...
                    <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                            <div>
                                ...
                                <p class="text-slate-600">Periksa kembali data invoice yang sudah terhubung dengan
                                    Packing List.</p>
                            </div>
                        </div>
                        ...
                        <div class="mt-6 grid gap-4 sm:grid-cols-2">
                            <div>
                                ...
                                <input type="text" readonly value="Packing List #{{ $invoice->packing_list_id }}"
                                    class="mt-2 w-full rounded-2xl border border-slate-300 bg-slate-100 px-4 py-3 text-slate-900" />
                            </div>
                            ...
                            <div>
                                ...
                                <input type="date" name="invoice_date"
                                    ...
                                    class="mt-2 w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-slate-900 outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100" />
                            </div>
                            ...
                            <table class="min-w-full divide-y divide-slate-200 text-sm">
                                ...
                                <tbody class="divide-y divide-slate-200 bg-white">
                                    ...
                                    <tr class="hover:bg-slate-50 transition">
                                        ...
                                        <td class="px-4 py-4 text-center text-slate-700">{{ $item->packaging_type }}
                                        </td>
                                        <td class="px-4 py-4 text-center text-slate-700">{{ $item->total_packaging }}
                                        </td>
                                        <td class="px-4 py-4 text-right text-slate-700">{{ number_format($item->weight,
                                            2, ',', '.') }}</td>
                                        <td class="px-4 py-4 text-right text-slate-700">Rp {{
                                            number_format($item->unit_price, 0, ',', '.') }}</td>
                                        <td class="px-4 py-4 text-right text-slate-900 font-semibold">Rp {{
                                            number_format($item->subtotal_price, 0, ',', '.') }}</td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            </table>
                    ...
                    <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                        ...
                        <div class="mt-6 grid gap-4 sm:grid-cols-2">
                            <div class="rounded-3xl border border-slate-200 bg-slate-50 p-5">
                                ...
                                <p class="mt-2 text-xl font-semibold text-slate-900">Rp {{
                                    number_format($invoice->total_value, 0, ',', '.') }}</p>
                            </div>
                            <div class="rounded-3xl border border-slate-200 bg-slate-50 p-5">
                                <p class="text-sm text-slate-500">Total Berat</p>
                                <p class="mt-2 text-xl font-semibold text-slate-900">{{
                                    number_format($invoice->total_weight, 2, ',', '.') }} kg</p>
                            </div>
                            <div class="rounded-3xl border border-slate-200 bg-slate-50 p-5">
                                <p class="text-sm text-slate-500">Tarif / kg</p>
                                <p class="mt-2 text-lg font-semibold text-slate-900">Rp {{
                                    number_format($invoice->packingList->shipment->price_per_kg ?? 0, 0, ',', '.') }} /
                                    kg</p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-slate-700">Biaya Tambahan
                                    (opsional)</label>
                                <input id="delivery_fee" name="delivery_fee" type="number" min="0" step="0.01"
                                    value="{{ old('delivery_fee', $invoice->delivery_fee) }}"
                                    class="mt-2 w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-slate-900 outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100" />
                            </div>
                            <div class="rounded-3xl border border-slate-200 bg-slate-50 p-5">
                                <p class="text-sm text-slate-500">Biaya Transport (Tarif × Berat)</p>
                                <p id="transport_base_display" class="mt-2 text-xl font-semibold text-slate-900">Rp {{
                                    number_format(($invoice->packingList->shipment->price_per_kg ?? 0) *
                                    $invoice->total_weight, 0, ',', '.') }}</p>
                            </div>
                            <div class="rounded-3xl border border-slate-200 bg-slate-50 p-5">
                                <p class="text-sm text-slate-500">Grand Total</p>
                                <p id="grand_total" class="mt-2 text-xl font-semibold text-emerald-700">Rp {{
                        ...
                        <div class="mt-6 grid gap-4 sm:grid-cols-2">
                            <div class="rounded-3xl border border-slate-200 bg-slate-50 p-5">
                                ...
                                <p id="ppn_amount" class="mt-2 text-lg font-semibold text-slate-900">Rp {{
                                    number_format($invoice->ppn_amount, 0, ',', '.') }}</p>
                            </div>
                            <div class="rounded-3xl border border-slate-200 bg-slate-50 p-5">
                                <p class="text-sm text-slate-500">PPH 2%</p>
                                <p id="pph_amount" class="mt-2 text-lg font-semibold text-slate-900">Rp {{
                                    number_format($invoice->pph_amount, 0, ',', '.') }}</p>
                            </div>
                        </div>
                    </section>
                        ...
                        <div class="mt-6 grid gap-4 md:grid-cols-2">
                            <div>
                                ...
                                <select name="payment_status" required
                                    ...
                                    <option value="{{ $value }}" {{ old('payment_status', $invoice->payment_status) ===
                            ...
                            <div>
                                ...
                                <input name="bank_name" type="text" value="{{ old('bank_name', $invoice->bank_name) }}"
                                    class="mt-2 w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-slate-900 outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100" />
                            </div>
                            <div>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea name="notes" rows="4"
                                    class="mt-2 w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-slate-900 outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100">{{ old('notes', $invoice->notes) }}</textarea>
                            </div>
                            <div class="md:col-span-2">
                                <label class="block text-sm font-medium text-slate-700">Upload Bukti Pembayaran</label>
                                <input name="proof_of_payment" type="file" accept="image/*,.pdf"
                                    class="mt-2 w-full text-slate-900" />
                                @if($invoice->proof_of_payment)
                                <p class="mt-2 text-sm text-slate-600">File saat ini: <a
                                        href="{{ asset('storage/' . $invoice->proof_of_payment) }}" target="_blank"
                                        class="font-semibold text-emerald-700 hover:text-emerald-900">Lihat Bukti</a>
                                </p>
                                @endif
                            </div>
                        </div>
            ...
            <aside class="space-y-6">
                <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                    ...
                    <div class="space-y-4 text-slate-700">
                        ...
                        <div class="flex justify-between gap-4">
                            ...
                            <strong id="card-grand-total" class="text-emerald-700">Rp {{
                ...
                <div class="rounded-3xl border border-slate-200 bg-slate-50 p-6">
                    ...
                    <p class="mt-3 text-sm text-slate-600 leading-relaxed">Update biaya pengiriman, status pembayaran,
                        dan upload bukti bayar jika sudah tersedia.</p>
                </div>
            </aside>
        </div>
    </div>
