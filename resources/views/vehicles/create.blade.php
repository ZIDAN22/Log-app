...
<div class="min-h-screen bg-slate-100 py-6 px-3 sm:px-5 lg:px-6">
    <div class="mx-auto max-w-[1100px]">
        <div class="mb-8 flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            <div>
                <h1 class="text-3xl font-bold tracking-tight text-slate-900">Tambah Kendaraan</h1>
                <p class="mt-2 text-slate-600">Isi data kendaraan baru dengan tampilan dashboard warehouse yang bersih.</p>
            </div>
        ...
        <div class="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
            <h2 class="mb-6 text-xl font-bold text-slate-900">Form Kendaraan</h2>
            <form action="{{ route('vehicles.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="flex flex-col gap-3 sm:flex-row sm:justify-end">
                    ...
                    <button type="submit"
                        class="inline-flex items-center justify-center rounded-2xl bg-emerald-600 px-6 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-emerald-700">Simpan Kendaraan</button>
                </div>
            </form>
        </div>
    </div>
