...
<div class="min-h-screen bg-slate-50 py-8 px-4 sm:px-6 lg:px-8">
    <div class="max-w-6xl mx-auto">
        <div class="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
                <h1 class="text-3xl font-bold text-slate-900 mb-2">Buat Packing List</h1>
                <p class="text-slate-600">Isi detail packing list untuk shipment dan verifikasi warehouse.</p>
            </div>
            <a href="{{ route('packing-list.index') }}" class="inline-flex items-center justify-center rounded-xl border border-slate-300 bg-white px-5 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50">
                Kembali ke Daftar
            </a>
        </div>
        ...
        <form method="POST" action="{{ route('packing-list.store') }}" class="space-y-8 bg-white rounded-3xl border border-slate-200 p-6 shadow-sm">
            @csrf

            <div class="grid gap-6 lg:grid-cols-3">
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2">Pilih Shipment</label>
                    <select name="shipment_id" required class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-slate-900 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100">
                        <option value="">-- Pilih Shipment --</option>
                        @foreach($shipments as $shipment)
                        <option value="{{ $shipment->id }}" {{ old('shipment_id') == $shipment->id ? 'selected' : '' }}>
                            {{ $shipment->receipt_number }} • {{ $shipment->invoice_number }} • {{ $shipment->sender_name }} → {{ $shipment->receiver_name }}
                        </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="packing_date" value="{{ old('packing_date', now()->toDateString()) }}" required class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-slate-900 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100" />
                </div>

                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2">Catatan Ringkas</label>
                    <textarea name="notes" rows="3" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-slate-900 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100">{{ old('notes') }}</textarea>
                </div>
            </div>
            ...
            <div class="rounded-3xl border border-slate-200 bg-slate-50 p-6">
                <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-6">
                    <div>
                        ...
                        <p class="text-slate-600 text-sm">Tambahkan semua item yang akan masuk ke packing list.</p>
                    </div>
                ...
                <div class="overflow-x-auto">
                    <table id="packing-items" class="min-w-full divide-y divide-slate-200 text-sm">
                        ...
                        <tbody class="divide-y divide-slate-200">
                            ...
                            @endphp
                            @if(count($oldItems) === 0)
                            @php $oldItems = [['item_name' => '', 'packaging_type' => 'Box', 'total_packaging' => 1, 'qty' => 1, 'weight' => 0, 'unit_price' => 0, 'item_notes' => '']]; @endphp
                            @endif
            ...
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-end">
                <a href="{{ route('packing-list.index') }}" class="inline-flex items-center justify-center rounded-2xl border border-slate-300 bg-white px-6 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50 transition">Batal</a>
                <button type="submit" class="inline-flex items-center justify-center rounded-2xl bg-slate-900 px-6 py-3 text-sm font-semibold text-white hover:bg-slate-800 transition">Simpan Packing List</button>
            </div>
        </form>
    </div>
</div>
