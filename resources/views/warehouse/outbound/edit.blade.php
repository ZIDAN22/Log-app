    ...
    <div class="mx-auto w-full max-w-screen-2xl">
        ...
        <div class="mb-5 flex flex-col gap-4 border-b border-slate-200 pb-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
                ...
                <h1 class="mt-1 text-2xl font-bold tracking-tight text-slate-950 sm:text-3xl">Edit Outbound</h1>
                <p class="mt-2 max-w-2xl text-sm leading-6 text-slate-600">Perbarui detail outbound yang sudah dibuat.</p>
            </div>
        ...
        <div class="grid gap-6 xl:grid-cols-3">
            <div class="xl:col-span-2 rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm">
                <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <h2 class="text-xl font-bold text-slate-900">Informasi Packing List</h2>
                        <p class="mt-1 text-sm text-slate-500">Data packing list yang akan digunakan untuk outbound.</p>
                    </div>
                    <span class="inline-flex items-center rounded-full px-4 py-2 text-sm font-semibold {{ $outbound->statusBadge() }}">{{ $outbound->status ?? 'Outbound' }}</span>
                </div>
                ...
                <div class="mt-8 overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
                    <div class="border-b border-slate-200 px-6 py-5">
                        <h3 class="text-lg font-bold text-slate-900">Daftar Barang</h3>
            ...
            <div class="space-y-6">
                <div class="rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm">
                    <h2 class="mb-4 text-xl font-bold text-slate-900">Ringkasan</h2>
                ...
                <div class="rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm">
                    <h2 class="mb-4 text-xl font-bold text-slate-900">Aksi</h2>
                    <p class="text-sm text-slate-600">Perbarui detail outbound pada form di bawah.</p>
                </div>
            </div>
        </div>
        ...
        <form method="POST" action="{{ route('warehouse.outbound.update', $outbound) }}" class="mt-8 rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div class="mb-6 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <h2 class="text-xl font-bold text-slate-900">Form Edit Outbound</h2>
            ...
            <div class="grid gap-6 lg:grid-cols-2">
                ...
                <div id="sea-field" class="transition duration-200">
                    ...
                    <input type="text" name="sea_shipping" value="{{ old('sea_shipping', $outbound->shipment?->sea_shipping ?? '') }}" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3.5 text-sm text-slate-900 transition focus:border-blue-500 focus:outline-none focus:ring-4 focus:ring-blue-100" placeholder="Masukkan nama kapal/rute" />
                ...
                <div id="sea-date-field" class="transition duration-200">
                    ...
                    <input type="date" name="sea_departure_date" value="{{ old('sea_departure_date', $outbound->shipment?->sea_departure_date?->format('Y-m-d') ?? '') }}" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3.5 text-sm text-slate-900 transition focus:border-blue-500 focus:outline-none focus:ring-4 focus:ring-blue-100" />
                ...
                <div id="airline-field" class="transition duration-200">
                    ...
                    <input type="text" name="air_shipping" value="{{ old('air_shipping', $outbound->shipment?->air_shipping ?? '') }}" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3.5 text-sm text-slate-900 transition focus:border-blue-500 focus:outline-none focus:ring-4 focus:ring-blue-100" placeholder="Masukkan nama maskapai" />
                ...
                <div id="air-date-field" class="transition duration-200">
                    ...
                    <input type="date" name="air_departure_date" value="{{ old('air_departure_date', $outbound->shipment?->air_departure_date?->format('Y-m-d') ?? '') }}" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3.5 text-sm text-slate-900 transition focus:border-blue-500 focus:outline-none focus:ring-4 focus:ring-blue-100" />
            ...
            <div class="mt-8 flex flex-wrap items-center gap-3">
                ...
                <a href="{{ route('warehouse.outbound.show', $outbound) }}" class="inline-flex items-center justify-center rounded-2xl border border-slate-300 bg-white px-6 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">Batal</a>
            </div>
        </form>
